...
[<Test>]
let ``Should produce message if annotation from querystring not found`` () =
  let vocabs = [{Property = "vocab:property"
                 Root = Term {t with
                                  Label = "Vocab Label";
                                  Children = [Term {t with
  ...
  let qsWithTwoVocabTerms = "vocab%3Aproperty=Uri%2Flong-guid-1&vocab%3Aproperty=Uri%2Fnot-found"

  let errorMessage = startServerWith {baseConfig with Vocabs = vocabs}
                     |> getQuery "/annotationtool/toyaml" qsWithTwoVocabTerms
                     |> CQ.select ".message"
                     |> CQ.text
  errorMessage |> should equal "Annotation not found."
