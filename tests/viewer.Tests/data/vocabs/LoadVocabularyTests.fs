...
[<Test>]
let ``Should retrieve the label from filters based on the Shortened Uri`` () =
  ...
  let filters = [{Vocab = "notused"; TermUri = "long-guid-1"}]

  let expectedLabels = ["Term1"]
  let actualResult = getLabelsByGuid vocabs filters
  actualResult |> should equal expectedLabels

[<Test>]
let ``Should retrieve more than one label from several filters provided`` () =
  let vocabs = [{Root = Term {
                              Uri = Uri.from "http://testing.com/Uri3"
                              ShortenedUri = "unknown"
                              Label = "Care home"
  ...
  let filters = [ 
                {Vocab = "notused"; TermUri = "long-guid-1"};
                {Vocab = "notused"; TermUri = "long-guid-2"}
                ]

  let expectedLabels = ["Term1"; "Term2";]
  let actualResult = getLabelsByGuid vocabs filters
  actualResult |> should equal expectedLabels

[<Test>]
let ``Should return an empty string when guid is not found`` () =
  let vocabs = [{Root = Term {
                              Uri = Uri.from "http://testing.com/Uri3"
                              ShortenedUri = "unknown"
                              Label = "Care home"
